...
const OpenGroup = async (group) => {
    ...
    try {
        ...
        console.warn("group from OpenGroup");
        console.warn(group);
...
const OpenFolder = async (folder) => {
    console.warn("Mapitem -> OpenFolder -> folder")
    console.warn(folder)
    ...
    try {
        ...
        const getFolderResponse= await response.json();
        console.warn("Mapitem -> OpenFolder -> getFolderResponse")
        console.warn(getFolderResponse)
        setList(getFolderResponse.children)
            ...
            (isGroup ? (
                <div className=' justify-start'>
                    <button key={index} onContextMenu={(e) => handleRightClick(e,elementOfList)} onClick={() => OpenGroup(elementOfList)} className='flex flex-col justify-items-center m-6' >
                        ...
                        <div className='h-5 w-9'>{elementOfList.groupName}</div>
                    </button>
                </div>
            ):(
                ...
                <button key={index} onContextMenu={(e) => handleRightClick(e,elementOfList)} onClick={() => OpenFolder(elementOfList)} className='flex flex-col justify-items-center m-6'  >
                    ...
                    <div className='h-5 w-9' >{elementOfList.name}</div>
                </button>
            )
        )))}
    </div>
