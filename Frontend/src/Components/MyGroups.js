...
export const MyGroups = (dataFromMyGroups) =>{
    ...
    useEffect(()=>{
        ...
        }).then((response) => response.json()).then((shareFolder) => {
            setShareFolder(shareFolder.children);
        }).catch((error) => {
    ...
    const NewGroup = ()=>{
        ...
        fetch('http://localhost:8080/api/share/createMemberGroup', {
            method: 'POST',
            headers: {
            'Content-Type': 'application/json',
            'Authorization': 'Bearer '+serverToken
            ...
            console.error("shareFolder newShareFolder");
    ...
    const OpenItem = async(item)=>{
        ...
        try {
            ...
            setShareFolder(shareFolderResponse);
            dataFromMyGroups.dataFromMyGroups(shareFolder,item)
            } catch (error) {
            console.error('Error retrieving shareFolder:', error);
            }
    }
    return (
        <div className="flex flex-row items-start">
            ...
            <div className=' flex justify-start m-12 mt-10 w-2/4 flex-wrap'>
                {shareFolder.children.map((item, index) => (
